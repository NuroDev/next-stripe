...
async function NextStripeHandler(
	...
	if (!req.query.nextstripe)
		return res.status(500).end(`Error: Cannot find [...nextstripe].js in pages/api/stripe`);
...
export default function NextStripe(
	...args: [NextApiRequest, NextApiResponse, BaseRouteOptions] | [BaseRouteOptions]
) {
	if (args.length === 1)
		return (req: NextApiRequest, res: NextApiResponse) => NextStripeHandler(req, res, args[0]);
